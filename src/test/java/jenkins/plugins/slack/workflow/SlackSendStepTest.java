...
public class SlackSendStepTest {
    ...
    public void testStepOverrides() throws Exception {
        ...
        slackSendStep.setUsername("slackUser");
        slackSendStep.setIconEmoji(":iconEmoji:");
        ...
        when(stepExecution.getSlackService(anyString(), anyString(), anyString(), anyString(), anyString(), anyString())).thenReturn(slackServiceMock);
        when(slackServiceMock.publish(anyString(), anyString())).thenReturn(true);

        stepExecution.run();
        verify(stepExecution, times(1)).getSlackService("teamDomain", "token", "tokenCredentialId", "channel", "slackUser", ":iconEmoji:");
    ...
    public void testValuesForGlobalConfig() throws Exception {
        ...
        when(slackDescMock.getUsername()).thenReturn("globalSlackUser");
        when(slackDescMock.getIconEmoji()).thenReturn("globalIconEmoji");
        ...
        stepExecution.run();
        verify(stepExecution, times(1)).getSlackService("globalTeamDomain", "globalToken", "globalTokenCredentialId", "globalChannel", "globalSlackUser", "globalIconEmoji");
        ...
        assertNull(stepExecution.step.getUsername());
        assertNull(stepExecution.step.getIconEmoji());
    }
